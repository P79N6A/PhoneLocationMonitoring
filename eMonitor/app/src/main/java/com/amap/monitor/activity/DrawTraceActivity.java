...
import com.amap.monitor.util.LogUtil;
import com.amap.monitor.util.WalkUtil;
...
public class DrawTraceActivity extends BaseActivity implements LocationSource, AMapLocationListener {
    ...
    public static final String LOCATION_DATA_URL = SERVER_URL+"/monitor/location.php?act=upload";
...
//
                ...
                username.setText("您的名字:"+Common.getUsername(DrawTraceActivity.this));
                TextView address = (TextView)findViewById(R.id.address);
                if(!aMapLocation.getAddress().equals("")){
                    address.setText("当前位置:"+aMapLocation.getAddress());
                }


                final String MapLocation = Common.AMapLocationToString(aMapLocation)+"#empid="+Common.getEmpId(DrawTraceActivity.this);
                Common.runInChildThread(new Runnable() {
                    @Override
                    public void run() {
                        HttpUtil.doPost(LOCATION_DATA_URL,MapLocation);
                    }
                });
